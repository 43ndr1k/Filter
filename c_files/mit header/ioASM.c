            ...
            for (j = 0; j < spalte; j++) {
				if(strcmp(typVonPpm,"P3") == 0) {
					...
					*/
					...
					R[i][j] = asmfarbeinvertieren (R[i][j], hell);
					G[i][j] = asmfarbeinvertieren (G[i][j], hell);
					B[i][j] = asmfarbeinvertieren (B[i][j], hell);
			} 
			else {	
				/*				
					int g = Grau[i][j];
					Grau[i][j] =  hell - g;
				*/
				Grau[i][j] = asmfarbeinvertieren (Grau[i][j], hell);
				}
